...
TEST(seraphis_wallet, store_and_load_key_container)
{
    ...
    // 3. save keys to file
    sp::jamtis::JamtisKeys &keys = kc_all.get_keys(chacha_key);
    ASSERT_TRUE(write_encrypted_file(wallet_file_all.string(), chacha_key, keys));
    // ASSERT_TRUE(write_encrypted_file(wallet_file_vo.string(), chacha_key, keys));
