...
//-------------------------------------------------------------------------------------------------------------------
// INTERNAL
static jamtis_mock_keys ser_keys_to_jamtis(ser_jamtis_mock_keys const &keys)
{
    return jamtis_mock_keys{
        .k_m = keys.k_m,
        .k_vb = keys.k_vb,
        .xk_ua = keys.xk_ua,
        .xk_fr = keys.xk_fr,
...
//-------------------------------------------------------------------------------------------------------------------
bool jamtis_keys_valid(jamtis_mock_keys const &keys)
{
    jamtis_mock_keys test_keys{keys};
...
//-------------------------------------------------------------------------------------------------------------------
KeyContainer::KeyContainer(jamtis_mock_keys &&keys, const crypto::chacha_key &key)
    : m_keys{keys}, m_encrypted{false}, m_encryption_iv{}
{
    encrypt(key);
}
//-------------------------------------------------------------------------------------------------------------------
KeyContainer::KeyContainer(jamtis_mock_keys &&keys, bool encrypted, const crypto::chacha_iv encryption_iv)
...
{
    ser_KeyContainer key_container;

    if (!read_encrypted_file(path, key, key_container))
        return {};

    if (!jamtis_keys_valid(key_container.keys))
    {
        return boost::none;
    }
...
//-------------------------------------------------------------------------------------------------------------------
const jamtis_mock_keys &KeyContainer::get_keys()
{
    return m_keys;
}
//-------------------------------------------------------------------------------------------------------------------
void KeyContainer::set_keys(jamtis_mock_keys &&keys, const crypto::chacha_key &key)
{
    m_keys = keys;
    m_encrypted = false;
...
bool KeyContainer::write_view_only(const std::string &path, const crypto::chacha_key &key)
{
    ...
    ser_jamtis_mock_keys view_only_keys = {
        .k_m = {},
        .k_vb = {},
        .xk_ua = {},
        .xk_fr = m_keys.xk_fr,
    ...
    //     .keys = ser_jamtis_mock_keys{m_keys},
...
bool KeyContainer::write_view_balance(const std::string &path, const crypto::chacha_key &key)
{
    ...
    jamtis_mock_keys view_only_keys;
    view_only_keys = jamtis_mock_keys{
        .k_m = {},
        .k_vb = m_keys.k_vb,
        .xk_ua = {},
        .xk_fr = m_keys.xk_fr,
