...
BLOB_SERIALIZER(ser_jamtis_mock_keys);
...
{
public:
    KeyContainer(jamtis_mock_keys &&keys, const crypto::chacha_key &key);

    KeyContainer() : m_keys{}, m_encryption_iv{}, m_encrypted{false} {}

    KeyContainer(jamtis_mock_keys &&keys,
        bool encrypted,
    ...
    /// get the keys
    const jamtis_mock_keys &get_keys();

    /// set keys
    void set_keys(jamtis_mock_keys &&keys, const crypto::chacha_key &key);
...
private:
    ...
    epee::mlocked<jamtis_mock_keys> m_keys;
    bool m_encrypted;
};
